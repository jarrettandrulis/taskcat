...
def stage_in_s3(buckets, project_name, project_root, dry_run=False):
    ...
    pool = ThreadPool(32)
    func = partial(_sync_wrap, project_name=project_name, project_root=project_root, dry_run=dry_run)
    pool.map(func, distinct_buckets.values())
    pool.close()
    pool.join()


def _sync_wrap(bucket, project_name, project_root, dry_run=False):
    S3Sync(bucket.s3_client, bucket.name, project_name, project_root, bucket.object_acl, dry_run=False)
